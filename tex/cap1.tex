...
Una \textbf{variabile aleatoria} (reale) $X$ associa ad ogni elemento $\chi$  di uno spazio degli esiti $\Omega$ un valore $x$ appartenente a uno spazio misurabile $(\mathbb{R})$. Nel caso di una variabile aleatoria reale si ha quindi:
\begin{equation}
...
L'obiettivo della meccanica statistica è determinare la probabilità che il sistema si trovi in un determinato stato microscopico $\mathbf{x}$ di energia $\mathcal{H}(\mathbf{x})$. Tale probabilità è descritta dalla \textbf{distribuzione di Boltzmann} ricavabile tramite il principio di massima entropia, come mostrato nell'Appendice A.
...
\paragraph*{Passo locale e globale}
...
Le due strategie discusse in questa sezione possono non essere alternative e diventare complementari. Un \textbf{Normalizing Flow (NF)} (Capitolo \ref{ch:nf}) addestrato ad approssimare $p(\mathbf{x})$ costituisce infatti un candidato naturale come \textbf{distribuzione di proposta} per un MCMC: pur non fornendo un campionamento diretto esatto, il flow può fungere da motore per mosse globali all'interno di una catena di Markov. Il metodo ibrido così ottenuto - \textbf{Flow-MCMC}, discusso nella Sezione \ref{sec:flow-mcmc} - combina la correttezza asintotica garantita dalla struttura markoviana con la capacità di esplorazione globale offerta dal flow.
